//
//  MoleculesDownloadView.swift
//  BioNeighbor
//
//  Molecules download section
//

import SwiftUI

struct MoleculesDownloadView: View {
    @StateObject private var backendService = BackendService.shared
    @State private var stats: DatabaseStats?
    @State private var downloadCount = 1000
    @State private var selectedSource = "pubchem"
            ...
            VStack(alignment: .leading, spacing: 24) {
                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Download \(downloadCount) more molecules")
                            .font(.headline)
                        
                        HStack {
                            Slider(value: Binding(
                                get: { Double(downloadCount) },
                                set: { downloadCount = Int($0) }
                            ), in: 100...10000, step: 100)
                            
                            TextField("Count", value: $downloadCount, format: .number)
                    ...
                    Button(action: downloadByCount) {
                        HStack {
                            ...
                            Text("Download \(downloadCount) Molecules")
                        }
                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        TextField("Enter molecule name...", text: $searchText)
                            .textFieldStyle(.roundedBorder)
                    ...
                    Button(action: downloadByName) {
                        HStack {
                            ...
                            Text("Download Selected Molecules")
                        }
                        .frame(maxWidth: .infinity)
                    }
                    .buttonStyle(.borderedProminent)
    ...
    private func performSearch() {
        ...
        Task {
            do {
                let results = try await backendService.searchMolecules(query: searchText, limit: 20)
                await MainActor.run {
    ...
    private func downloadByCount() {
        ...
        Task {
            do {
                let response = try await backendService.downloadMolecules(count: downloadCount, source: selectedSource)
                await MainActor.run {
    ...
    private func downloadByName() {
        ...
        Task {
            do {
                let response = try await backendService.downloadMolecules(names: names)
                await MainActor.run {
                    if let taskId = response.taskId {
                        currentTaskId = taskId
                        startStatusPolling(taskId: taskId)
