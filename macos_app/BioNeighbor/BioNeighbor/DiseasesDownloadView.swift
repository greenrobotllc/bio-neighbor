//
//  DiseasesDownloadView.swift
//  BioNeighbor
//
//  Diseases download section
//

import SwiftUI

struct DiseasesDownloadView: View {
    ...
    @State private var bulkCount = 100
            ...
            VStack(alignment: .leading, spacing: 24) {
                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        TextField("Enter disease name...", text: $searchText)
                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Download top \(bulkCount) diseases")
                            .font(.headline)
    ...
    private func performSearch() {
        ...
        Task {
            do {
                let results = try await backendService.searchDiseases(query: searchText, limit: 20)
                await MainActor.run {
    ...
    private func downloadByName() {
        ...
        Task {
            do {
                let response = try await backendService.downloadDiseases(names: names)
                await MainActor.run {
    ...
    private func downloadBulk() {
        ...
        Task {
            do {
                let response = try await backendService.downloadDiseases(count: bulkCount)
                await MainActor.run {
                    if let taskId = response.taskId {
                        currentTaskId = taskId
                        startStatusPolling(taskId: taskId)
